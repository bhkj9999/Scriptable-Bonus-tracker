...
class Widget extends Base {
  ...
  constructor (arg) {
    super(arg)
    this.name = 'Cashback tracker'
    this.desc = 'for your credit card'
    this.fonts_style = {
        big_title: Font.title1(),
        bank_title: Font.boldRoundedSystemFont(18),
        small_bank_title: Font.headline(),
        little_bank_title: Font.boldRoundedSystemFont(12),
        body: Font.italicSystemFont(12),
        little_body: Font.italicSystemFont(9),
  ...
  async render () {
    ...
    const {data, result}= await this.getData()
    switch (this.widgetFamily) {
      case 'small':
        return await this.renderSmall(data)
      case 'medium':
        return await this.renderMedium(data, result)
      default:
        return await this.renderLarge(data, result)
    }
  }
  ...
  async renderSmall (data) {
    ...
    const nextUpdateDate = dF.string(this.nextUpdateDateObj);
    const nextUpdateDateString = this.textMap.s_update + nextUpdateDate;
  
    const citi_title = w.addText("Citi");
    const citi_text = w.addText(data.citi);
    const chase_title = w.addText("CFF");
    const chase_text = w.addText(data.chase);
    const discover_title = w.addText("Discover");
    const discover_text = w.addText(data.discover);
    w.addSpacer();
    const datetext = w.addText(nextUpdateDateString);
    datetext.rightAlignText();
    discover_title.font = this.fonts_style.little_bank_title;
    discover_text.font = this.fonts_style.little_body;
    chase_title.font = this.fonts_style.little_bank_title;
    chase_text.font = this.fonts_style.little_body;
    citi_title.font = this.fonts_style.little_bank_title;
    citi_text.font = this.fonts_style.little_body;
    datetext.font = this.fonts_style.mono_font;
    return w;
  }
  /**
   * 渲染中尺寸组件
   */
  async renderMedium (data, result) {
    let w = new ListWidget();
    let dF = new DateFormatter();
    dF.dateFormat = "HH:mm MMM dd yyyy";
    const nextUpdateDate = dF.string(this.nextUpdateDateObj);
    const nextUpdateDateString = this.textMap.update + nextUpdateDate;
    const discover_title = w.addText("Discover IT");
    const discover_text = w.addText(data.discover);
    const citi_title = w.addText("Citi Dividend");
    const citi_text = w.addText(data.citi);


    const chase_title = w.addText("Chase Freedom Flex");
    const chase_text = w.addText(data.chase);

    w.addSpacer();
    const lastLine = w.addStack();
    const refreshResultText = lastLine.addText(`${this.textMap.req}: ${this.textMap.status[result]}`);
    lastLine.addSpacer();
    const datetext = lastLine.addText(nextUpdateDateString);
    datetext.rightAlignText();
    discover_title.font = this.fonts_style.small_bank_title;
    discover_text.font = this.fonts_style.body;
    chase_title.font = this.fonts_style.small_bank_title;
    chase_text.font = this.fonts_style.body;
    citi_title.font = this.fonts_style.small_bank_title;
    citi_text.font = this.fonts_style.body;
    refreshResultText.font = this.fonts_style.mono_font;
    datetext.font = this.fonts_style.mono_font;
    return w;
  }
  /**
  ...
  async renderLarge (data, result) {
    ...
    const big_title = w.addText(this.textMap.bt);
    w.addSpacer(10);
    const discover_title = w.addText("Discover It");
    const discover_text = w.addText(data.discover);
    w.addSpacer(3);
    ...
    w.addSpacer();
    const refreshResultText = w.addText(`${this.textMap.req}: ${this.textMap.status[result]}`);
    w.addSpacer(3);
    const datetext = w.addText(nextUpdateDateString);
    refreshResultText.rightAlignText();
    datetext.rightAlignText();
    big_title.font = this.fonts_style.big_title;
    discover_title.font = this.fonts_style.bank_title;
    discover_text.font = this.fonts_style.body;
    chase_title.font = this.fonts_style.bank_title;
    chase_text.font = this.fonts_style.body;
    citi_title.font = this.fonts_style.bank_title;
    citi_text.font = this.fonts_style.body;
    refreshResultText.font = this.fonts_style.mono_font;
    datetext.font = this.fonts_style.mono_font;
    return w;
   ...
   */
  async getData () {
    const api = "https://raw.githubusercontent.com/bhkj9999/Scriptable-Bonus-tracker/main/data.json";
